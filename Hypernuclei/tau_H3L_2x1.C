...
void tau_H3L_2x1(int config=0){
  ...
  TPad *p1 = new TPad("p1","",0.0,0.0,0.4,1.0);
  ...
  for(int i=0;i<NC;i++) {
    drawText(380, i+1.02, Label[i], 42, 0.04);
  }
  
  drawText(140, y2*0.94, Form("Ave. = %5.1f #pm %3.1f (S=%3.1f)", muAve, sigAve_wt, scale), 42, 0.04);
  ...
  // ideogram
  //////////////
  TPad *p2 = new TPad("p2","",0.4,0.0,1.0,1.0);
  ...
  drawText(430, ideo->GetMaximum()*0.03, Form("Confidence Level"), 42, 0.03);
  drawText(500, ideo->GetMaximum()*(-0.03), Form(" = %5.3f", TMath::Prob(chi2_tot, nn-1)), 42, 0.03);
  ...
  c11->Update();
  c11->SaveAs(Form("fig/tau_H3L_2x1_%d.pdf", config));
  c11->SaveAs(Form("fig/tau_H3L_2x1_%d.png", config));
