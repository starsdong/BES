...
void plotH3LAcceptSnn()
{
  ...
  for(int i=0;i<NE;i++) {
    ...
    fun[i]->SetParameters(mH3L,eta_ref[1],-ybeam[i]);
  ...
  TF1 *fun_a = new TF1("fun_a",Perp,-ybeam[0],x2,3);
  fun_a->SetParameters(mH3L,eta_ref[0],-ybeam[0]);
  fun_a->SetRange(-ybeam[0],-eta_ref[0]-ybeam[0]);
  fun_a->SetLineColor(kColor[0]);
  fun_a->SetLineStyle(4);
  fun_a->Draw("same");
  ...
  drawText(1.25, 2.7, "3.0 GeV", 52, 0.045, 85, kBlack);
  drawText(0.25, 1.2, "3.0 GeV (no iTPC)", 52, 0.035, 70, kBlack);
  drawText(0.75, 2.7, "4.5 GeV", 52, 0.045, 85, kRed);
  drawText(0.2, 2.7, "7.7 GeV", 52, 0.045, 85, kBlue);

  drawText(-1.3, 0.7, "Target", 32, 0.04, 0, kBlack);

  drawText(-1.6, 3.05, "{}^{3}_{#Lambda}H", 42, 0.065);

  c1->SaveAs("fig/acceptH3LSnn.pdf");
  c1->SaveAs("fig/acceptH3LSnn.png");
