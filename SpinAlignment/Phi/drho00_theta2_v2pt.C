...
void drho00_theta2_v2pt(const Char_t *RcName = "Rc4")
{
  style();
  ...
  for(int iv2 = 0;iv2<NV2;iv2++) {
    ...
    for(int i=0;i<NPt;i++) {
      c1->cd(i+1+iv2*NPt);
      fMc[iv2][i] = ((TH2D *)fin[iv2]->Get("hPtCosTheta2"))->ProjectionY(Form("Mc_%d_%d",iv2, i),i_edge[i]+1,i_edge[i+1]);
      if(iv2==0) {
	fRc[iv2][i] = ((TH2D *)fin[iv2]->Get(Form("hPtCosTheta2Rc3")))->ProjectionY(Form("Rc_%d_%d",iv2, i),i_edge[i]+1,i_edge[i+1]);
      } else {
	fRc[iv2][i] = ((TH2D *)fin[iv2]->Get(Form("hPtCosTheta2%s",RcName)))->ProjectionY(Form("Rc_%d_%d",iv2, i),i_edge[i]+1,i_edge[i+1]);
      }
  ...
  c2->SaveAs(Form("fig/drho00_theta2_pT_v2pt_%s.pdf",RcName));
  c2->SaveAs(Form("fig/drho00_theta2_pT_v2pt_%s.png",RcName));

  TFile *fout = new TFile(Form("root/drho00_CosTheta2_pT_v2pt_%s.root",RcName),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_rc[i]->Write();
  }
