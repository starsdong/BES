...
void a2_y_v2pt()
{
  ...
  c1->Update();
  c1->SaveAs(Form("fig/a2_y_fit_v2pt.pdf"));
  c1->SaveAs(Form("fig/a2_y_fit_v2pt.png"));  
  ...
  c2->SaveAs(Form("fig/a2_y_v2pt.pdf"));
  c2->SaveAs(Form("fig/a2_y_v2pt.png"));
  ...
  TFile *fout = new TFile(Form("root/drho_a2_y_v2pt.root"),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_drho[i]->Write();
    for(int j=0;j<NY;j++) {
  ...
  c3->SaveAs(Form("fig/drho_a2_y_v2pt.pdf"));
  c3->SaveAs(Form("fig/drho_a2_y_v2pt.png"));
