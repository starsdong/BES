...
void drho00_dy(const Double_t sigY = 9.9)
{
  style();
  ...
  c2->SaveAs(Form("fig/Cos2PhiRP_dy_Y_%3.1f.pdf", sigY));
  c2->SaveAs(Form("fig/Cos2PhiRP_dy_Y_%3.1f.png", sigY));

  TFile *fout = new TFile(Form("root/drho00_Cos2PhiRP_dy_Y_%3.1f.root", sigY),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_rc[i]->Write();
    gr_corr[i]->Write();
