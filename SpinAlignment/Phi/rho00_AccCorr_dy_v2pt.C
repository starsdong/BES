...
void rho00_AccCorr_dy_v2pt()
{
  ...
  c2->SaveAs(Form("fig/rho00_AccCorr_dy_v2pt.pdf"));
  c2->SaveAs(Form("fig/rho00_AccCorr_dy_v2pt.png"));
  
  TFile *fout = new TFile(Form("root/rho00_AccCorr_dy_v2pt.root"),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_corr[i]->Write();
  }
