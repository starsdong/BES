...
/////////////////////////////////////////////////////
void drho00_theta2_EPres(const Double_t sigY = 5.0)
{
  style();
  ...
  TH1D *fMc[NV2][NRes][NPt], *fRc[NV2][NRes][NPt];
  ...
  //  TFile *fin = new TFile(Form("accept_Sergei_v2_%3.1f_Y_%3.1f.root",v2, res, sigY));
  TFile *fin[NV2][NRes];
  for(int iv2 = 0;iv2<NV2;iv2++) {
	...
	fMc[iv2][ires][i] = ((TH2D *)fin[iv2][ires]->Get("hPtCosTheta2"))->ProjectionY(Form("Mc_%d_%d_%d",iv2, ires, i),i_edge[i]+1,i_edge[i+1]);
	fRc[iv2][ires][i] = ((TH2D *)fin[iv2][ires]->Get("hPtCosTheta2Rc3"))->ProjectionY(Form("Rc_%d_%d_%d",iv2, ires, i),i_edge[i]+1,i_edge[i+1]);
	...
	fRc[iv2][ires][i]->Draw("same");
	
	v2[iv2][ires][i] = 2.5 * (fMc[iv2][ires][i]->GetMean() - 1./3.);
	v2e[iv2][ires][i] = 2.5 * fMc[iv2][ires][i]->GetMeanError();
	v2Rc[iv2][ires][i] = 2.5 * (fRc[iv2][ires][i]->GetMean() - 1./3.);
	v2eRc[iv2][ires][i] = 2.5 * fRc[iv2][ires][i]->GetMeanError();
  ...
  c2->Draw();
  TH2D *h2 = new TH2D("h2","",1,0.0,5.0,1,-0.014, 0.028);
  ...
  const Int_t markerStyle[NRes] = {24, 20};//, 21, 22};
  for(int i=0;i<NV2;i++) {
    for(int j=0;j<NRes;j++) {
      gr[i][j] = new TGraphErrors(NPt, pT, v2[i][j], 0, v2e[i][j]);
      gr[i][j]->SetName(Form("drho00_CosTheta2_Mc_%d_%d",i,j));
  ...
  for(int i=0;i<NV2;i++) {
    for(int j=0;j<NRes;j++) {
      gr_rc[i][j] = new TGraphErrors(NPt, pT1, v2Rc[i][j], 0, v2eRc[i][j]);
      gr_rc[i][j]->SetName(Form("drho00_CosTheta2_Rc_%d_%d",i,j));
  ...
  leg->SetLineColor(10);
  for(int j=NRes-1;j>=0;j--) {
    leg->AddEntry(gr_corr[0][j], Form("#sigma_{#Psi} = %3.1f", resName[j]), "pl");
  }
  leg->Draw();
  */
  drawText(2.3, 0.024, "MC");
  drawText(3.2, 0.024, "RC", 42, 0.05, 0, 2);
  //  drawText(4.0, 0.024, "RC corr.", 42, 0.05, 0, 4);
  drawHistBox(0., 5.0, -0.014, 0.028);

  c2->Update();

  c2->SaveAs(Form("fig/drho00_theta2_pT_Res_Y_%3.1f.pdf", sigY));
  c2->SaveAs(Form("fig/drho00_theta2_pT_Res_Y_%3.1f.png", sigY));
  
  TFile *fout = new TFile(Form("root/drho00_CosTheta2_pT_Res_Y_%3.1f.root", sigY),"recreate");
  for(int i=0;i<NV2;i++) {
    for(int j=0;j<NRes;j++) {
      gr[i][j]->Write();
      gr_rc[i][j]->Write();
