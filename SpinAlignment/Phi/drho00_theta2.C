...
/////////////////////////////////////////////////////
void drho00_theta2(const Double_t sigY = 9.9)
{
  style();
  ...
  for(int iv2 = 0;iv2<NV2;iv2++) {
    ...
    for(int i=0;i<NPt;i++) {
      ...
      fRc[iv2][i] = ((TH2D *)fin[iv2]->Get("hPtCosTheta2Rc3"))->ProjectionY(Form("Rc_%d_%d",iv2, i),i_edge[i]+1,i_edge[i+1]);
  ...
  c2->SaveAs(Form("fig/drho00_theta2_pT_Y_%3.1f.pdf", sigY));
  c2->SaveAs(Form("fig/drho00_theta2_pT_Y_%3.1f.png", sigY));
  
  TFile *fout = new TFile(Form("root/drho00_CosTheta2_pT_Y_%3.1f.root", sigY),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_rc[i]->Write();
  }
