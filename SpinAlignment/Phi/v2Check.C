...
void v2Check(const Double_t sigY = 9.9)
{
  ...
  const Int_t NV2 = 3;
  const Double_t v2Name[NV2] = {0.0, 0.1, 0.2};
  ...
  TGraphErrors *gr[NV2];
  const Int_t markerStyle[NV2] = {24, 20, 21};
  for(int i=0;i<NV2;i++) {
  ...
  c2->SaveAs(Form("fig/v2_pT_Y_%3.1f.pdf", sigY));
  c2->SaveAs(Form("fig/v2_pT_Y_%3.1f.png", sigY));
