...
// Check different acceptance effect
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//void a2_Accept_v2pt(const Int_t iAccept = 3, const Double_t sigY = 9.9)
void a2_Accept_v2pt(const Int_t iAccept = 3)
{
  style();
  ...
  for(int iv2 = 0;iv2<NV2;iv2++) {
    ...
    for(int i=0;i<NPt;i++) {
      ...
      fMc[iv2][i]->Rebin(20);
      cout << Form("hPtCos2Phi%s",RCName[iAccept]) << endl;
      fRc[iv2][i] = ((TH2D *)fin[iv2]->Get(Form("hPtCos2Phi%s",RCName[iAccept])))->ProjectionY(Form("Rc_%d_%d",iv2, i),i_edge[i]+1,i_edge[i+1]);
      fRc[iv2][i]->Rebin(20);
	...
	eff_err(m, n, &eff[j], &err[j]);
      ...
      drawText(-0.4, eff[0]/0.7*0.9, Form("v_{2} = %3.1f", v2[iv2][i]), 42, 0.08);
  ...
  c1->Update();
  c1->SaveAs(Form("fig/a2_fit_Accept_%d_v2pt.pdf", iAccept));
  c1->SaveAs(Form("fig/a2_fit_Accept_%d_v2pt.png", iAccept));  
  ...
  TGraphErrors *gr[NV2];
  const Int_t markerStyle[NV2] = {24, 20}; //{24, 20, 21, 22};
  for(int i=0;i<NV2;i++) {
  ...
  for(int i=0;i<NV2;i++) {
    leg->AddEntry(gr[i], Form(" v_{2} = %s", v2Name[i]));
  }
  leg->Draw();
  drawHistBox(0., 5.0, -0.1, 0.01);
  c2->Update();

  c2->SaveAs(Form("fig/a2_Accept_%d_v2pt.pdf", iAccept));
  c2->SaveAs(Form("fig/a2_Accept_%d_v2pt.png", iAccept));
  ...
  TFile *fout = new TFile(Form("root/drho_a2_Accept_%d_v2pt.root", iAccept),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_drho[i]->Write();
    for(int j=0;j<NPt;j++) {
  ...
  c3->SaveAs(Form("fig/drho_a2_Accept_%d_v2pt.pdf", iAccept));
  c3->SaveAs(Form("fig/drho_a2_Accept_%d_v2pt.png", iAccept));
