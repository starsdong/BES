...
void drho00_theta2_y_v2pt()
{
  ...
  c2->SaveAs(Form("fig/drho00_theta2_y_v2pt.pdf"));
  c2->SaveAs(Form("fig/drho00_theta2_y_v2pt.png"));
  
  TFile *fout = new TFile(Form("root/drho00_CosTheta2_y_v2pt.root"),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_rc[i]->Write();
  }
