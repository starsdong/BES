...
void a2_dy_v2pt()
{
  ...
  c1->Update();
  c1->SaveAs(Form("fig/a2_dy_fit_v2pt.pdf"));
  c1->SaveAs(Form("fig/a2_dy_fit_v2pt.png"));  
  ...
  for(int i=0;i<NV2;i++) {
    leg->AddEntry(gr[i], Form(" v_{2} = %s", v2Label[i]));
  }
  leg->Draw();
  drawHistBox(0., 1.0, -0.3, 0.05);
  c2->Update();

  c2->SaveAs(Form("fig/a2_dy_v2pt.pdf"));
  c2->SaveAs(Form("fig/a2_dy_v2pt.png"));
  ...
  TFile *fout = new TFile(Form("root/drho_a2_dy_v2pt.root"),"recreate");
  for(int i=0;i<NV2;i++) {
    gr[i]->Write();
    gr_drho[i]->Write();
    for(int j=0;j<NY;j++) {
  ...
  c3->SaveAs(Form("fig/drho_a2_dy_v2pt.pdf"));
  c3->SaveAs(Form("fig/drho_a2_dy_v2pt.png"));
