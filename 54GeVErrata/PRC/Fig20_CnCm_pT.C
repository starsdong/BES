...
void Fig20_CnCm_pT()
{
  ...
  for(int i=0;i<NE;i++) {
	...
	if(i<pad_gap_index) {
	  ...
	  drawSysError(gr_sys[i][j][k], 0.06, (ymax2[j]-ymin2[j])*0.02, kColor[k]);
	}
	...
	if(i<pad_gap_index) {
	  ...
	  drawText(xmin+(xmax-xmin)*0.3, (ymax2[j]-ymin2[j])*0.88+ymin2[j], Form("%s GeV", EneLabel[i]), 42, 0.14);
	}
      }
  ...
  for(int i=0;i<NE;i++) {
    double xx_1 = i*1.*(pad_x2-pad_x1-pad_x_gap)/(1.0-pad_x1)/NE + 0.018;
    double xx_2 = i*1.*(pad_x2-pad_x1-pad_x_gap)/(1.0-pad_x1)/NE + 0.054;
    double xx_3 = i*1.*(pad_x2-pad_x1-pad_x_gap)/(1.0-pad_x1)/NE + 0.090;
    if(i>=pad_gap_index) {
      xx_1 += pad_x_gap*(pad_x2-pad_x1)/(pad_x2-pad_x1-pad_x_gap);
      xx_2 += pad_x_gap*(pad_x2-pad_x1)/(pad_x2-pad_x1-pad_x_gap);
      xx_3 += pad_x_gap*(pad_x2-pad_x1)/(pad_x2-pad_x1-pad_x_gap);
    }
    
    drawText(xx_1, 0.75, "1.2", 42, 0.33);
    drawText(xx_2, 0.75, "1.6", 42, 0.33);
    drawText(xx_3, 0.75, "2.0", 42, 0.33);
  }
  ...
  drawText(0.01, 0.6, "STAR Au+Au Collisions 0-5% most central", 42, 0.345);
  drawText(0.12, 0.18, "0.4 < p_{T} < p_{T}^{max} (GeV/c), |y| < 0.5", 42, 0.33);
  pad_coll->Update();
  ...
  c1->Update();
  c1->SaveAs("fig/Fig20_CnCm_pT.pdf");
  c1->SaveAs("fig/Fig20_CnCm_pT.png");
